...
import { QUARTER_LABELS, calculatePension, ageOn, rmdStartAge } from '../calpers';
    ...
    <div className="p-2 sm:p-6 max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card title="About You">
          ...
          <SliderField label="Retirement age" name="retirementAge" value={pension.ageYears} min={Math.max(50, formula.minAge)} max={75} step={1}
          ...
          <NumberField label="Your Social Security (monthly, at your start age)" name="yourSS" value={config.yourSS} step={10} prefix="$" onChange={onChange}
            hint="From your statement at ssa.gov/myaccount. Enter 0 if you don't get Social Security." />
        ...
        <Card title="CalPERS Pension" wide>
          <div>
            ...
            <Hint>
              It's on your CalPERS Annual Member Statement and in myCalPERS. Minimum retirement age {formula.minAge}
              {formula.maxPercent !== null ? ` · capped at ${pct(formula.maxPercent, 0)} of final compensation` : ''}.
              {' '}<a href={formula.source} target="_blank" rel="noreferrer" className="text-[#0072B2] underline">CalPERS chart</a>
            </Hint>
          </div>
          ...
          <Toggle label="Calculate the pension from the formula" name="pensionFromFormula" checked={config.pensionFromFormula} onChange={onChange}
            hint="Off: type in the monthly amount from your myCalPERS estimate instead." />
          ...
          {config.pensionFromFormula ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <NumberField label="Final compensation (monthly)" name="finalCompensation" value={config.finalCompensation} step={1} prefix="$" onChange={onChange}
                hint="Classic members: highest 12 consecutive months of pay (some employers use 36). PEPRA members: highest 36 months, subject to the PEPRA pay limit." />
              <NumberField label="Service credit (years)" name="serviceCreditYears" value={config.serviceCreditYears} step={0.001} onChange={onChange}
                hint="From your Annual Member Statement or myCalPERS." />
              ...
              <SliderField label="Beneficiary option factor" name="beneficiaryOptionFactor" value={Number((config.beneficiaryOptionFactor * 100).toFixed(1))} min={70} max={100} step={0.1}
                ...
                hint="Your option amount ÷ the unmodified amount on a myCalPERS estimate. 100% = unmodified (no survivor benefit)." />
            </div>
          ) : (
          ...
          <SliderField label="Pension COLA" name="pensionCOLA" value={Number((config.pensionCOLA * 100).toFixed(2))} min={0} max={5} step={0.25}
            ...
            hint="Most CalPERS contracts have a 2% COLA; some employers contracted for more. It also grows Social Security and the spouse pension here." />

          {config.pensionFromFormula && <>
            <div className="bg-gray-50 rounded p-4 text-sm text-gray-700 space-y-1">
              {pension.eligible ? (
                <>
                  <p>Age at retirement: <b>{pension.ageYears}y {pension.ageMonths}m</b> → {QUARTER_LABELS[pension.quarter]} column · factor <b>{pct(pension.ageFactor, 3)}</b></p>
                  ...
                  <p>Unmodified: <b>{$(pension.unmodified)}</b>/mo · With your option: <b className="text-[#009E73]">{$(pension.optionAmount)}</b>/mo</p>
                </>
              ) : (
                <p className="text-[#D55E00] font-semibold">Under this formula's minimum retirement age ({formula.minAge}) on this date.</p>
              )}
              <p className="text-xs text-gray-500 pt-1">Check this against a myCalPERS retirement estimate. If they differ, trust CalPERS and adjust final compensation or switch the formula off and type in their number.</p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs text-right border-collapse">
                <thead>
                  <tr className="bg-gray-100 text-gray-600">
                    ...
                    {QUARTER_LABELS.map(q => <th key={q} className="p-1.5">{q}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {formula.table.map((row, r) => (
                    <tr key={row.age} className={row.age === pension.tableAge ? 'bg-blue-50' : 'border-t border-gray-100'}>
                      <td className="p-1.5 text-left font-semibold">{row.age}{r === formula.table.length - 1 ? '+' : ''}{row.age < formula.minAge ? ' *' : ''}</td>
                      {row.factors.map((f, q) => {
                        const active = pension.eligible && row.age === pension.tableAge && q === pension.quarter;
              ...
              {formula.table[0].age < formula.minAge && <Hint>* Below the minimum retirement age of {formula.minAge} (50 with combined classic and PEPRA service).</Hint>}
            </div>
          </>}
        </Card>
        ...
        <Card title="Retiree Health">
          ...
          <Hint>From the CalPERS health plan rates for your region and plan (monthly). You pay whatever is above the employer's contribution, plus Medicare Part B.</Hint>
